...
Outcome
 A — real control. cpufreq present, userspace available, writes change scaling_cur_freq. Build a real frequency setter; only power is estimated.
 B — no real control. Directory missing or writes are silent no-ops. Build a simulated cpufreq layer behind the same interface. Modules 2 and 3 are unaffected.
